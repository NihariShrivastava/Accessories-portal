...
export const CashierDetailsView = ({
  ...
  cashierReport: CashierReport, onBack: () => void
}) => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      setLoading(true);
      try {
        // Fetch the cashier profile to get assigned counters
        const { data: profileData } = await supabase.from('profiles').select('assigned_counters').eq('id', cashierReport.cashier_id).single();
  ...
  return (
    <div className="space-y-6">
      <ViewHeader 
        title={`Drawer Details: ${cashierReport.cashier_name}`} 
        onBack={onBack} 
        ...
        <div className="p-4 border-b border-border">
          ...
          <div className="flex justify-center gap-1.5 mt-3">
            {tabsList.map((tab) => (
              <div 
                key={tab.id}
                className={`w-1.5 h-1.5 rounded-full transition-colors ${tab.id === tableTab ? 'bg-primary' : 'bg-border'}`}
              />
            ))}
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-border">
                {tableTab === 'pending' && pendingHandovers.length === 0 && (
                  <tr><td colSpan={4} className="px-4 py-12 text-center text-muted-foreground text-sm">No pending handovers found.</td></tr>
                )}
